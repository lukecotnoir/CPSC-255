...
public class TestRecursiveFunctionsInClass {

	public static void main(String[] args) {
		// Test Kirsky numbers.
		String userinput = JOptionPane.showInputDialog("How many Kirsky numbers do you want? ");
		int howmany = Integer.parseInt(userinput);
		for (int num =1 ; num <= howmany; ++num) {
			System.out.println(num + "'st Kirsky number is: " + RecursiveFunctionsInClass.kirsky(num));  // awkward English
		}
		...
		{
			if (RecursiveFunctionsInClass.contains(words1,s,0,words1.size()))
		...
		for (String s : lookfor)
		{
			if (RecursiveFunctionsInClass.contains(words2,s,0,words2.size()))
		...
		for (String s : lookfor)
		{
			if (RecursiveFunctionsInClass.contains(words3,s,0,words3.size()))
		...
		for (String s : lookfor)
		{
			if (RecursiveFunctionsInClass.contains(words4,s,0,words4.size()))
...
// testing max
		...
		{
			int answer = RecursiveFunctionsInClass.max(onecase, 0, onecase.length);
			System.out.print(answer + " is the max number in: ");
